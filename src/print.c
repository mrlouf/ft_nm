...
/*   Updated: 2026/02/18 15:33:42 by nicolas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int symbol_should_be_skipped(t_symbol *sym, unsigned char flags)
{
	if (sym->type == 'U' && sym->name[0] == '\0')
		return 1; // Skip undefined global symbols with null name
...
{	
	...
	else if (sym->type == 'a' || sym->type == 'A'
		|| sym->type == 'n' || sym->type == 'N'
		|| sym->type == 'r' || sym->type == 'R'
		|| sym->type == 'd' || sym->type == 'D') {
